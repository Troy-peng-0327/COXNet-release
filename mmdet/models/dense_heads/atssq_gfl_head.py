...
class ATSSQGFLHead(AnchorHead):
    ...
    def __init__(self,
        ...
        super(ATSSQGFLHead, self).__init__(
            num_classes,
            in_channels,
    ...
    def _init_layers(self):
        ...
        self.atss_reg = nn.Conv2d(
            self.feat_channels,
            self.num_base_priors * 4 * (self.reg_max + 1),
            self.pred_kernel_size,
            padding=pred_pad_size)
    ...
    def _get_target_single(self,
        ...
        bbox_preds = self.integral(bbox_preds)
        # 这里可以在每一个lvl上乘以一个stride，然后再decode
        start = 0
        decoded_bboxes_list = []
        for lvl, (num_lvl_anchors, stride) in enumerate(zip(num_level_anchors, self.anchor_generator.strides)):
            end = start + num_lvl_anchors
            lvl_bbox_preds = bbox_preds[start:end]
            lvl_anchors = anchors[start:end]

            # 在每个层级上乘以 stride 然后解码
            lvl_bbox_preds *= stride[0]
            decoded_bboxes = self.bbox_coder.decode(lvl_anchors, lvl_bbox_preds)
            decoded_bboxes_list.append(decoded_bboxes)

            start = end

        dec_bbox_preds = torch.cat(decoded_bboxes_list, dim=0)
    ...
    def _get_bboxes_single(self,
        ...
        for level_idx, (cls_score, bbox_pred, score_factor, stride, priors) in \
                enumerate(zip(cls_score_list, bbox_pred_list,
                              score_factor_list, self.anchor_generator.strides, mlvl_priors)):
