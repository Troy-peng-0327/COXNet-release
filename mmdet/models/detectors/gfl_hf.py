...
class GFLHF(SingleStageDetector):
    ...
    def extract_feat(self, img):
        ...
        v_img, t_img = img
        # save_img(v_img, 'img_v_ori', path='gfl_hf_r50_fpn_1x')
        # save_img(t_img, 'img_t_ori', path='gfl_hf_r50_fpn_1x')
...
def save_feature_to_img(features, name, timestamp, method='cv2', channel=None, output_dir=None, maxmin=None):
    ...
    if output_dir is None:
        output_dir = 'work_dir/vis/gfl_hf_r50_fpn_1x'
...
def save_img(img, name, save_root = 'work_dir/vis/', path='gfl_hf_r50_fpn_1x'):
    import matplotlib.pyplot as plt
    image = img.cpu().squeeze().permute(1, 2, 0)
    image = (image - image.min()) / (image.max() - image.min())
    save_path = f'{save_root}/{path}/{name}.jpg'
    plt.imshow(image)
    plt.axis('off')  # 不显示坐标轴
